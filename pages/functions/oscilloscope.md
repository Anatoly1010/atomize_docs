...
It should be noted that for all available Keysight oscilloscopes, the exact number of points somehow depends on the [offset](#oscilloscope_horizontal_offseth_offset), [timebase](#oscilloscope_timebasetimebase), and acquisition [type](#oscilloscope_acquisition_typeac_type). In order to ensure the desired number of points, this function attempt to change the [timebase](#oscilloscope_timebasetimebase). If after 1 minute the number of points still differs, the function will be interrupted and a corresponding warning will be printed. In the case of interruption, the timebase will be returned to its initial value.<br>
