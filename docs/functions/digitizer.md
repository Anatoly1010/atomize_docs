...
The keyword `live_mode` controls the accumulation behaviour. In the default mode (`live_mode=0`) every transferred buffer is summed into the running on-board average, so the returned arrays correspond to all the data accumulated so far. In `live_mode=1` every returned curve is a snapshot of one complete phase cycle and is intended for live plotting of a single point. The accumulators are cleared when the first buffer after the previous snapshot arrives, and the buffers are summed until every phase of the cycle has been received. When one ADC buffer holds fewer detection windows than the phase cycle, a snapshot spans several buffers and the function returns `None, None` until it is complete. The keyword `skip_redundant` affects how repeated packets of the same point are treated: with the default `False` every parsed packet contributes to the running average (correct on-board-averaging semantics), while `True` makes only the first packet of each consecutive run of the same point contribute. If no new buffer has been transferred from the card at the time of execution the function returns `None, None` (see also [`digitizer_at_exit()`](#digitizer_at_exit)).
